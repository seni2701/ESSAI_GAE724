...
# --- 3) PRÉPARATION DES INDICES DE TÉLÉDÉTECTION (SIMULATION) ---
print(f"\n{'='*70}")
print("AJOUT DES INDICES DE VÉGÉTATION")
print("="*70)
print("NOTE: Simulation temporaire des indices (à remplacer par données réelles Sentinel-2)\n")
...
print("Indices simulés:")
for idx in ['NDVI', 'NDWI', 'EVI', 'LAI']:
    print(f"  - {idx}: min={df[idx].min():.2f}, max={df[idx].max():.2f}, mean={df[idx].mean():.2f}")
...
# --- 11) Définition du modèle TabResNet (ARCHITECTURE ULTRA-LÉGÈRE) ---
